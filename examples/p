...
extract_snapshot.py
-------------------
Sample an OpenFOAM wedge case at a target time (default t = 0.2, the
steady field) onto a uniform Cartesian grid in the (x, y) plane and save
the resampled fields as a compressed NumPy archive (NPZ).

Usage:
    python3 extract_snapshot.py             # extract all four grids
    python3 extract_snapshot.py --grid Fine # one specific grid
    python3 extract_snapshot.py --time 0.2 --nx 600 --ny 240

Output goes to ./snapshots/snap_<label>_t<time>.npz with arrays
    x, y, p, rho, t, label, valid, (optionally Ux, Uy, Umag, T)
Cells inside the solid wedge body (below the ramp) are masked with NaN.

Requirements: pyvista >= 0.43 (handles both OpenFOAMReader and
POpenFOAMReader transparently via pv.get_reader).

See the example README, section "Producing snapshots", for exactly which
files must exist before this runs.
"""
...
from data import GRIDS, T_BENCHMARK, DOMAIN_X, DOMAIN_Y, in_wedge_body
...
def extract_one(foam_path: Path, label: str, t_target: float,
    ...
    # Mask the solid wedge body (below the ramp line).
    valid = ~in_wedge_body(X, Y)
